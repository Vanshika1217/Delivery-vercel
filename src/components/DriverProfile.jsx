...
const DriverProfile = () => {
  ...
  const fetchDriverProfile = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:5000/api/partners/me', {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
      console.log(response.data); // You are seeing this in console
      setDriver(response.data);   // ✅ This was missing
  ...
  const handleSaveClick = async () => {
    ...
    try {
      await axios.put("localhost:5000/api/driver/me", updatedData, {
    ...
    >
      ...
      {editingField === fieldName ? (
        ...
        <div className="flex justify-between items-center">
          ...
          >
            <img src={ FiStar} className="h-5 w-5 opacity-70 hover:opacity-100" alt="edit" />
          </button>
        </div>
      )}
    </motion.div>
    ...
    <div className="p-6 md:p-10 bg-gray-50 min-h-screen">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
          <AnimatePresence>
            {renderEditableField("Name", "name", driver.username, <FiUser size={20} />, "name")}
            {renderEditableField("Vehicle", "vehicle", driver.vehicleType, <FaCar size={20} />, "vehicle")}
